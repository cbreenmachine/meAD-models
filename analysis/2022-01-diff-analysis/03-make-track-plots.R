...
parser$add_argument("--smoothing_window", default = 0)
...
plot_roi_ix <- function(ix){
    ...
    all.tracks <- append(append(top.list, gene.track), bottom.list)
    ofile <- create_output_name(odir, nCG, CHR, a, p, roi.df$name[ix])
    print(ofile)
    plot_and_save(a-p, b+p, all.tracks, ofile)
}
...
for (i in 1:nrow(roi.df)){
    start <- roi.df$start[i]
    end <- roi.df$end[i]
    tmp <- roi.df$name[i]

    ofile <- file.path(odir, paste0("dmr-", tmp, ".png"))
    plot_dmr(diffs.df, start,end,ofile)
}
