...
df.tmp <- mutate(df, start = pos, end = pos + 1, chr = test.cohort$chr[1]) %>%
            filter(lfdr < 0.01)



.annotate_to_genome <- function(df, genes, window=1000){

    df.gr <- GRanges(df) 
    genes.gr <- GRanges(genes)
    overlap <- findOverlaps(df.gr + window, genes.gr)

    # Subset based on if they overlap
    genes.gr.2 <- genes.gr[subjectHits(overlap), ]
    df.gr.2 <- df.gr[queryHits(overlap), ]

    # Compute distance between DMR and 10kb gene
    dd <- as.data.frame(distanceToNearest(df.gr.2, genes.gr.2))$distance

    # DMR locus, distance, gene.locus, gene.name
    df <- as.data.frame(df.gr.2, row.names = NULL) %>%
        mutate(dmr.locus = .format_locus(seqnames, start, end))
    df$distance <- dd
    ann.df <- as.data.frame(genes.gr.2, row.names = NULL) %>%
        transmute(gene.locus = .format_locus(seqnames, start, end), gene.name)
    comb.df <- cbind(df, ann.df)
    return(comb.df)
}

ann <- .annotate_to_genome(df.tmp, genes)
...
ann.df <- ann %>%
    ...
    filter(gene.name != "") %>%
    arrange(-N)
write_csv(ann.df, file = file.path(args$idir, "lfdr-genes-tally.csv"))
