...
parser$add_argument('--ifile', default = "../../dataSummaries/controlLOAD-pc2-ct5/pvals.bed", help = "input directory.")
parser$add_argument('--odir', default = "../../figs/controlLOAD-pc2-ct5/", help = "output directory.")
args <- parser$parse_args()

ofile_prefix <- file.path(args$odir, Sys.Date())
...
ix <- which(sample(df$p.corrected) > 0.01)
...
p <- sub.df[!ix.to.ignore, ]  %>%
  ggplot(aes(x = bp_cum, color = as.factor(chrom), y = -log10(p.corrected))) +
...
ofile <- paste0(ofile_prefix, "-manhattanAdjusted.png")
cowplot::save_plot(plot = p, filename=ofile, 
                   base_width = 7, base_height = 5)


# Volcano Plot ------------------------------------------------------------
# ix <- which(sample(df$p.corrected) > 0.2)
# ix.to.ignore <- ix[1:floor(length(ix) * 0.997)]

# Base plot--handle points, colors, etc.
p <- df[!ix.to.ignore, ] %>%
  dplyr::filter(is.finite(-log10(p.corrected))) %>%
  mutate(Significance = ifelse(p.corrected < alpha_bonferroni, "Significant", "Not Significant")) %>%
  ggplot(aes(
    x = diagnostic_group_coded, y = -log10(p.corrected), 
    color = Significance)) +
  geom_point()+
  scale_color_manual(values = c("grey", my_pal(3)[3])) +
...
ofile <- paste0(ofile_prefix,  "-volcano.png")
cowplot::save_plot(plot = p, filename=ofile, base_width = 7, base_height = 5)
