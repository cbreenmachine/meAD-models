...
#TODO run on batches of data at a time, not all at once.
...
parser$add_argument("--idir", default= "../../dataDerived/methylBedImputedSubset1/withoutMCI/")
args <- parser$parse_args()

load("../../dataReference/epicArrayReferenceForMethylCC.RData")
...
read_data <- function(idir, chrom, s) {
    out <- fread(file.path(idir, paste0(chrom, ".", s, ".bed"))) %>%
            ...
            dplyr::mutate(chrom = chrom, start = start) %>%  
...
est <- estimatecc(object = bs,  find_dmrs_object = epic_dmrs)

df = cbind("sample_id" = rownames(cell_counts(est)), cell_counts(est))

ofile <- file.path(args$idir, "bloodCellCompFromEPIC.csv")
write_csv(df, ofile)

# (default) Typically says that BSseq object contained 74 out of 600 celltype-specific regions
# (+1 chromStart) Typically says that BSseq object contained 79 out of 600 celltype-specific regions
# (-1 chromStart) Typically says that BSseq object contained 78 out of 600 celltype-specific regions