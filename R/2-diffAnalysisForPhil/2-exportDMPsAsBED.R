...
parser$add_argument("--dir", default= "../../dataDerived/methylBedImputedSubset1/withoutMCI/tests/has_risk_allele/", help='Where are the models stored')
args <- parser$parse_args()

all.files <-list.files(file.path(args$dir), pattern = "*model*", full=T)
...
df %>%
    dplyr::mutate(has_load = diagnostic_group_coded / 2) %>%
    group_by(has_load, has_risk_allele) %>%
    summarize(count = n()) %>%
    write_csv("2022-09-09-tallyForPhil.csv")
