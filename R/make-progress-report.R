...
ID.df <- read_xlsx("../data/meta/fromLigia/sample-IDs-WRAP-ADRC.xlsx") %>% mutate(RA.ID = `RA ID`)
samples.df <- read_csv("../data/DNAMethylation_data_04152022.csv") %>% left_join(ID.df, by="study_id")


all_files <- list.files('../data/', pattern = "bcf.csi", recursive=T)
processed_files <- str_remove(basename(all_files), ".bcf.csi")

samples.df %>%
    filter(RA.ID %in% processed_files) %>%
    group_by(diagnostic_group.x) %>%
    summarize(count = n())
...
total <- df %>% select_if(is.numeric) %>% colSums()
total$stage <- c("raw", "trimmed", "mapped", "called")
write_csv(as.data.frame(total), file =  paste0(Sys.Date(), "-total.csv"))
