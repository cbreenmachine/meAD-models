...
parser$add_argument("--idir", default= "../../dataDerived/methylBedImputed/withoutMCI/", help='Directory to run DSS on')
...
parser$add_argument("--covariates", default= "diagnostic_group_coded,bmi,age_at_visit,type1,type2,type3,type4,type5")
...
# If Control vs LOAD, code as 0 and 1
if (!args$include_MCI){
    master.df <- master.df %>%
        dplyr::filter(diagnostic_group != "MCI") %>%
        mutate(diagnostic_group_coded = ifelse(diagnostic_group == "CONTROL", 0, 1))
} else {
    # Code as 0, 1, 2
    master.df <- master.df %>%
...
blood.df <- read_csv(args$blood_file, show_col_types=F) %>% arrange(sample_id)
PCs <- read_csv(file.path(args$idir, paste0(args$chr, ".PCs.csv")), show_col_types=F) %>% arrange(sample_id)
...
df <- left_join(blood.df, master.df, by = "sample_id") %>%
        left_join(PCs, by = "sample_id") %>%
...
keep_cols <- c("chromStart", as.character(valid_sample_ids))
